...
\phantomsection
\addcontentsline{toc}{chapter}{Language Architecture}
...
These underlying grammar rules dictate the parsing boundary for the DSL structure depicted here. Specifically, the \texttt{program} rule requires that every processed script evaluates optional metadata definitions first (\texttt{config\_block} and \texttt{import\_decl}), while strictly mandating at least one \texttt{playbook\_decl} to form a valid execution graph.
...
The derivation tree is the most direct structural representation of how an IncidentFlow specification is recognized by the grammar. Whereas token streams capture lexical units and parser rules define admissible forms, the derivation tree materializes the full hierarchical decomposition from the start symbol to terminal constructs. In this project, the tree is used as an internal parser artifact for grammar validation, semantic mapping, and quality assurance in language evolution.
...
Overall, the derivation tree should be viewed as a parser-level intermediate representation, not as the core architecture of the final IncidentFlow product. Its role is to make syntax processing deterministic and verifiable, so that higher-level components can reliably translate authored playbooks into automation-ready reporting representations.
